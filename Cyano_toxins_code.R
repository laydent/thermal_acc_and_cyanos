...
funcc <- function(x){975.37*x} #updated function for calculating counts from RFU (data from 10/16/19)

dat <- dat %>% group_by(Starting_Cond,Acclim_temp,Acute_temp.id,Acute_temp,Nutrients,Replicate,RFU_raw_Final) %>% 
  mutate(counts_final = funcc(RFU_raw_Final)) %>%
  mutate(counts_initial = funcc(RFU_raw_initial)) %>%
  mutate(time=5) %>% #add in the time elapsed
  mutate(M0=0) %>% #add in starting toxin concentration
  mutate(loss=0.09) %>% # add in loss rate that descibes loss rate of toxins based on a half life of ~3weeks 
  mutate(x0 =(exp(loss*time)*Mc_adj-M0)* (loss+mu) /  (-1 + exp((loss*time)+(mu*time))*counts_initial)       )%>%#add in per cell toxin production, see "Tox_mod_12_17_19.nb" for analytical solution to x
  mutate(x1= 1.56831*Mc_adj*(0.09+mu)/ (-1+exp(0.45+(5*mu))*counts_initial      )   ) #should be same as above
...
g2<-ggplot(data.frame(dat),aes(x=as.factor(Acclim_temp),y=Mc_adj,fill=Tx.label, linetype=Medium,color=Tx.label))+
  ...
  scale_color_manual(values=c("#3399FF","gray60","#FF6666"))+
  theme_bw()+
...
#figure 2
ggsave(filename = '~/Output/Toxin_fig.png',g3,width = 7, height = 5, units = "in") 
...
g4<-grid.arrange(g2 + theme(legend.position = "none"),g1,bottom=legend,nrow=1)

ggsave(filename = '~/Output/Toxin_figs_SI.png',g4,width = 8, height = 5, units = "in") 
...
opfunc(tavg2$mc_avg[1],tavg2$mc_avg[2]) #nutrient-limited; cold, acc = 207.8% increase
opfunc(tavg2$mc_avg[5],tavg2$mc_avg[6]) #nutrient-replete; acc, hot = 17% decrease
opfunc(tavg2$mc_avg[3],tavg2$mc_avg[2]) #nutrient-limited; hot, acc = 74.6% decrease
